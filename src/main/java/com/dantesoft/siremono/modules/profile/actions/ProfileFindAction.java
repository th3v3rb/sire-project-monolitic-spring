...
import com.dantesoft.siremono.modules.auth.store.dto.RoleMapper;
...
public class ProfileFindAction extends AbstractCommand<ProfileFindInput, ProfileFindOutput> {
  ...
  protected ProfileFindOutput doExecute() {
    ...
    var profile = userProfileService.findByUser(account)
  ...
  private String makeSignedUrl(String name) {
    ...
    try {
      url = uploads.getPresignedUrl(bucket, name, 600);
    } catch (Exception ex) {
    }
    return url;
  }
